...
const realTimeAlerts = [
  { id: 1, time: 'Just now', message: 'Low stock predicted for Amoxicillin at City General in 3 days', type: 'critical' },
  { id: 2, time: '2 min ago', message: 'Surplus detected: 320 units of Paracetamol at Regional Medical', type: 'info' },
  { id: 3, time: '5 min ago', message: 'Redistribution complete: Insulin transferred to Children\'s Hospital', type: 'success' },
  { id: 4, time: '12 min ago', message: 'Seasonal demand spike predicted for antibiotics next month', type: 'warning' },
  { id: 5, time: '18 min ago', message: 'Stock levels normalized at University Hospital', type: 'success' },
  { id: 6, time: '25 min ago', message: 'Critical shortage averted: Metformin supply replenished', type: 'success' },
  { id: 7, time: '30 min ago', message: 'High demand alert: Ibuprofen usage up 40% in Pediatrics', type: 'warning' },
  { id: 8, time: '45 min ago', message: 'New shipment arrived at Central Warehouse', type: 'info' },
  { id: 9, time: '1 hour ago', message: 'Expiry risk: 50 units of Vaccine A expiring in 14 days', type: 'critical' },
  { id: 10, time: '2 hours ago', message: 'Automated order placed for Saline Solution', type: 'info' },
  { id: 11, time: '3 hours ago', message: 'Cold chain integrity verified for recent insulin delivery', type: 'success' },
  { id: 12, time: '4 hours ago', message: 'Anomalous consumption pattern detected in Ward 3', type: 'warning' },
  { id: 13, time: '5 hours ago', message: 'Regulatory compliance check passed for Storage B', type: 'success' },
  { id: 14, time: '6 hours ago', message: 'Low humidity warning in archival storage', type: 'warning' },
  { id: 15, time: '7 hours ago', message: 'Emergency backup generator test successful', type: 'info' },
];
...
const AIFeaturesSection = () => {
  ...
  const [alertIndex, setAlertIndex] = useState(0); // Keeping for the single banner alert
  const [page, setPage] = useState(0);

  // Simulate real-time alert rotation (Single Banner)
  useEffect(() => {
  ...
  // Auto-refresh feed pages
  useEffect(() => {
    const interval = setInterval(() => {
      setPage((prev) => (prev + 1) % Math.ceil(realTimeAlerts.length / 5));
    }, 5000);
    return () => clearInterval(interval);
  }, []);
  ...
  const currentFeedAlerts = realTimeAlerts.slice(page * 5, (page + 1) * 5);
    ...
    <section id="ai-features" className="py-20 bg-card">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <Card className={`lg:w-[400px] border-border transition-all duration-300 ${currentAlert.type === 'critical' ? 'bg-destructive/10 border-destructive/50' :
            ...
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <Bell className={`h-5 w-5 animate-pulse ${currentAlert.type === 'critical' ? 'text-destructive' :
            ...
            <CardContent>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {earlyAlerts.map((alert) => (
                  <div
                    key={alert.id}
                    className={`p-4 rounded-lg border transition-all hover:shadow-md ${alert.severity === 'critical' ? 'bg-destructive/5 border-destructive/30' :
              ...
              <div className="space-y-3 min-h-[300px]">
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="space-y-3"
                  >
                    {currentFeedAlerts.map((alert) => (
                      <div
                        key={alert.id}
                        className={`flex items-center gap-3 p-3 rounded-lg transition-all ${alert.type === 'critical' ? 'bg-destructive/10 border border-destructive/30' :
                      ...
                      >
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${alert.type === 'critical' ? 'bg-destructive' :
